...
const MODEL_ROOT = `${BASE}/logo-3d`;

const objModelPromises = new Map();

function loadObjWithMtl(rootDir) {
  const key = rootDir;
  if (objModelPromises.has(key)) return objModelPromises.get(key);

  const dir = rootDir.endsWith("/") ? rootDir : `${rootDir}/`;
  const promise = new Promise((resolve, reject) => {
    ...
    mtlLoader.load(
      ...
      (materials) => {
        ...
        objLoader.load("ax.obj", resolve, undefined, reject);
      },
      undefined,
      reject
    );
...
/** Стабільне освітлення: Phong з MTL часто виглядає «плоско» — Standard + DoubleSide. */
...
function LogoMesh() {
  const loaded = use(useMemo(() => loadObjWithMtl(MODEL_ROOT), []));
